...
with open('.\\Train.txt', 'w', encoding='UTF-8') as f:
    for i in range(0, 15):
        df = pd.read_excel("医生评论.xlsx", sheet_name=i)
        data1 = df['评价内容']
        data2 = df['分类']
...
class SoPmi:
    def __init__(self):
        ...
        self.train_path = './expand_data/Train.txt'
        self.candipos_path = './expand_data/candi_pos.txt'
        self.candineg_path = './expand_data/candi_neg.txt'
        self.sentiment_path = './expand_data/sentiment_words.txt'

    '''分词'''
    def seg_corpus(self, train_data, sentiment_path):
        #将情感词加入到用户词典当中，保证分词能够将种子情感词切开
        with open(sentiment_path, encoding='utf-8-sig') as text:
            sentiment_words = [line.strip().split('\t')[0] for line in text]
        for word in sentiment_words:
            jieba.add_word(word)
        seg_data = list()
        count = 0
        with open(train_data, encoding='utf-8-sig') as text:
            for line in text:
                line = line.strip()
                count += 1
                if line:
    ...
    def collect_candiwords(self, seg_data, cowords_list, sentiment_path):
        ...
        def collect_sentiwords(sentiment_path, word_dict):
            with open(sentiment_path, encoding='utf-8-sig') as text:
                pos_words = set([line.strip().split('\t')[0] for line in text if
                                line.strip().split('\t')[1] == 'pos']).intersection(set(word_dict.keys()))
                neg_words = set([line.strip().split('\t')[0] for line in text if
